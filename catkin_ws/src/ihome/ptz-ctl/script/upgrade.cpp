...
typedef struct
{
    ...
    CMD_TYPE		cmd;
    char	   		file_name[PARAM_SIZE_MAX];
    char	   		card_name[CARD_NAME_SIZE];
    char 			file_bin[CARD_NAME_SIZE];
    char			state;
    unsigned short  	total;
    unsigned short  	size;
    unsigned short  	new_version; 
    unsigned short  	old_version; 	
} t_upg_info;
...
const char		*g_usage	= "\n *******     The version is 2.2   ******* \
						\n Usage:\t upgrade [-opt] -s [slot] -f [file]  \
						\n -opt : \
						\n\tq : query card information .  \
						\n\tr : reconfig the fpga . \
						\n\tb : reboot NM system . \
						\n\tu : upgrade .\
						\n\th : show the usage.  \
...
/***************************************************
	 操作日志文件     
 ****************************************************/
void do_write_logs(void)
{
	FILE    *fp;
	time_t  timep;
	time(&timep);

	fp = fopen("/var/log/stm32-log.txt", "a+");
	...
	if(NULL != fp)
	{
		...
		{
			case UPGRADE:
				fprintf(fp, " upgrade------- time : %s ,       version : %s  \n"
						, ctime(&timep)
						, "v1.0"
						  );
				break;
			case BOOT:
				fprintf(fp, " reboot ------- time : %s \n"
						, ctime(&timep));
				break;
			default:
				break;
	...
	{
		printf("\n ****  ERR  :  Can't open upgrade logs file : stm32-log.txt !! \n");
	}
...
/***************************************************
	 异常信号扑获与处理
****************************************************/
void deal_abort(int sig)
{
	char    sel = 'N';

	printf("\n Please confirm quit this process : [Y/N] ? ");
	scanf("%c", &sel);
	if((sel == 'Y') || (sel == 'y'))
	{
		do_write_logs();
...
{
	(void)signal(SIGINT, deal_abort);   
	(void)signal(SIGTERM, deal_abort);   
	(void)signal(SIGQUIT, deal_abort);   
	(void)signal(SIGTSTP, deal_abort);   
}
//校验和
unsigned long checksum(unsigned char *pframe, unsigned short length)
{
...
int set_motor(void)
{
	...
	tx_buf[pos++] = 0x20;
	tx_buf[pos++] = 0x01;
	tx_buf[pos++] = 1;//channle
	tx_buf[pos++] = 2;//dir
	tx_buf[pos++] = 0x04;//pulse /low bit
	tx_buf[pos++] = 0x15;		//high bit
	tx_len =frm_pack(tx_buf, (pos-TABS_FM_HEAD));
	send_data(tx_buf, tx_len);
