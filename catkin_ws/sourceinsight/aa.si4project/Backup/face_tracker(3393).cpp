...
void chatterCallback(const robot_msgs::Ear::ConstPtr& msg)   //callback函数，收到msg时调用
{
	...
	if(cmd.compare("看看我是谁") == 0){
		send_cmd = 1;
	}else if(cmd.compare("你认识我吗") == 0){
		send_cmd = 2;
	}else if(cmd.compare("解锁") == 0){
		send_cmd = 3; 
	}
}
...
{ 
    ...
    chatter_pub = nh_.advertise<std_msgs::String>("/face_chat", 1);
	ear_sub = nh_.subscribe("/aiui_ear", 1, chatterCallback);
