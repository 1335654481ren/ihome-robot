...
//	int i;
    ...
    cJSON_AddItemToObject(root, "type", cJSON_CreateString("tts"));
    cJSON_AddItemToObject(root, "content", item);
    cJSON_AddItemToObject(item, "action", cJSON_CreateFalse());
...
int ask_aiui_todo(unsigned char type,const char *arg1,const char *arg2)
{
	...
	{
		case TODO_TTS_START:
			err = AiuiCtrl_tts_start((char*)arg1);
		break;
		case TODO_TTS_STOP:
			err = AiuiCtrl_tts_stop();
		break;
		case TODO_WAKTUP:
			err = AiuiCtrl_wakeup(true);
		break;
		case TODO_SLEPP:
			err = AiuiCtrl_wakeup(false);
		break;
		...
		case TODO_VOICE_OPEN:
			err = AiuiCtrl_Voice_EN(ENABLE);
		break;
		case TODO_VOICE_STOP:
			err = AiuiCtrl_Voice_EN(DISABLE);
		break;
		default:break;
	}
	return err;
